...
object StreamingKMean {
  ...
  def main(args: Array[String]) {
    ...
    val sqlDF = sqlContext.sql("SELECT key, `values`, rel, day(cast(cast(`timestamp` as BIGINT) as TIMESTAMP)) as day, from_unixtime(cast(`timestamp` as BIGINT), 'HH:mm:ss') as time FROM tmone.traffic_light limit 500")

    val trainingLines = sqlDF.map(t => "["+t(0)+","+t(2)+","+t(3)+","+t(4)+","+t(1)+"]")
